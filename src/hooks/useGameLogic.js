...
import { moveLetter as importedMoveLetter } from '../utils/moveLetterLogic'; // Importujeme funkciu, ktorá už NEBUDE posielať na server
...
function useGameLogic(socket, gameId, myPlayerIndex, slovakWordsArray, gameState, setGameState) {
    ...
    const moveLetter = useCallback((letterData, source, target) => {
        ...
        // Vola sa funkcia importedMoveLetter, ktora uz nebude volat server.
        // Namiesto toho vrati aktualizovany gameState, ktory potom aktualizujeme len lokalne
        const updatedGameState = importedMoveLetter({
            gameState,
            setGameState,
            myPlayerIndex,
            setJokerTileCoords,
        ...
        // Ak sa stav zmenil, aktualizujeme ho LOKÁLNE. Na server ho pošleme až pri potvrdení ťahu.
        if (updatedGameState) {
    ...
    const assignLetterToJoker = useCallback((selectedLetter) => {
        ...
        setIsActionInProgress(true);
        if (jokerTileCoords) {
            // Posiela sa len pri priradeni jokera, nie pri kazdom tahu.
            // Je to nutne, aby ostatni hraci videli, co bolo priradene.
        ...
        setIsActionInProgress(false); // Pridane, aby sa odblokovalo UI po dokonceni akcie.
    ...
    const confirmTurn = useCallback(() => {
        ...
        // Posielame na server, ale uz len raz, a to s finalnym stavom
        sendPlayerAction(socket, gameId, 'turnSubmitted', turnDetails);
        sendPlayerAction(socket, gameId, 'updateGameState', updatedGameState);
    }, [gameState, myPlayerIndex, socket, gameId, validWordsSet, isActionInProgress]); // Pridané isActionInProgress do závislostí
